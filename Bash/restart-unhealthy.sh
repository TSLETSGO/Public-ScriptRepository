...
if [ $(docker ps --filter "name=$CONTAINER_NAME" | grep -c "(healthy)") -eq 0 ] && [ $(docker ps --filter "name=$CONTAINER_NAME" | grep -c "(health: starting)") -eq 0 ]; then
        echo "$CONTAINER_NAME is non-healthy"
        while [ $(docker ps --filter "name=$CONTAINER_NAME" | grep -c "(healthy)") -eq 0 ]; do
                if [ $(docker ps --filter "name=$CONTAINER_NAME" | grep -c "starting") -eq 0 ]; then
